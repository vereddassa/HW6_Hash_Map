...
/**
 * This function is chacking if we can create a given string array
 * using words from another given string array.
 */
void checkMagazine(int magazine_count, char** magazine, int note_count,
    ...
    /*This is a counter that counts the number of uniq mached 
    between the arrays*/
    int word_matched = 0;

    for (int i = 0; i < note_count; ++i) {
        for (int j = 0; j < magazine_count; ++j) {
            if (!strcmp(note[i], magazine[j])) {
                /*If we are here we got a match, so we change 
                the word in the magazine array to avoid of 
                reusing the same word more than one time.*/
                magazine[j] = "#";
                word_matched += 1;
                break;
            }
